(function( $ ) {
    ...
    $.Shop.prototype = {
        ...
        this.cartPrefix = "Furniture-"; // Prefix string to be prepended to the cart's name in the session storage
            ...
            this.paypalCurrency = "ZAR"; // PayPal's currency code
            ...
            if (this.$subTotal[0].innerHTML != "R 0") {
                this.$element.find( "#checkout" ).removeAttr("disabled");
                this.$element.find( "#update-cart" ).removeAttr("disabled");
                this.$element.find( "#empty-cart" ).removeAttr("disabled");
            }

                else{
                this.$element.find( "#checkout" ).attr("disabled", "disabled"); 
                this.$element.find( "#update-cart" ).attr("disabled", "disabled");
                this.$element.find( "#empty-cart" ).attr("disabled","disabled");
                } 
                ...
                $( document ).on( "click", ".pdelete a", function( e ) {
                    ...
                    for( var i = 0; i < items.length; ++i ) {
                        ...
                        if( product == productName ) {
                            items.splice( i, 1 );
                            break; //govenp26 - stop removing when first item found
                        }
                    }
                    newItems = items;
                    var updatedCart = {};
            ...
            if( this.$formCart.length ) {
                ...
                if( items.length == 0 ) {
                    ...
                    for( var i = 0; i < items.length; ++i ) {
                        ...
                        var qty = item.qty;
                        var html = "<tr><td class='pname' style='text-align: center;'>" + product + "</td>" + "<td class='ppic'><img src='" + pic + "'style='max-width: 40%;'></td><td class='pqty'><input type='number' min='1' value='" + qty + "' class='qty'/></td>";
                ...
                if( cartItems.length > 0 ) {
                    ...
                    for( var j = 0; j < cartItems.length; ++j ) {
                        ...
                        var cartPic = cartItem.pic;
                        var cartPrice = this.currency + " " + cartItem.price;
                        var cartQty = cartItem.qty;
                        var cartHTML = "<tr><td class='pname' >" + cartProduct + "</td>" + "<td class='ppic'><img src='" + cartPic + "' style='max-width: 40%;'></td><td class='pqty'>" + cartQty + "</td>" + "<td class='pprice'>" + cartPrice + "</td></tr>";
          ...
          if( self.$updateCartBtn.length ) {
            self.$updateCartBtn.on( "click", function() {
                ...
                $rows.each(function() {
                    ...
                    var ppic = $row.find(".ppic")[0].innerHTML.replace("<img src=\"","").replace("\" style=\"max-width: 40%;\">","");
        ...
        handleAddToCartForm: function() {
            ...
            self.$formAddToCart.each(function() {
                ...
                var pic = $product.data( "pic" );
                var price = self._convertString( $product.data( "price" ) );
                var name =  $product.data( "name" );
                
                $form.on( "submit", function() {
//govenp26 - update only quatity if product already in cart
                    var cart = self._toJSONObject( self.storage.getItem( self.cartName ) );
                    var items = cart.items;
                    ...
                    for( var i = 0; i < items.length; ++i ) {
                        ...
                        if( prod == name ) {
                            currqty = item.qty;
                            currsubt = currqty * item.price;
                            self._removeFromCart(i);    
                            break; //govenp26 - stop searching when item found
                        }
                    }


                    var qtyn = parseInt($form.find( ".qty" ).val()) + currqty;
                    var qty = self._convertString(qtyn);
                    
                    var subTotal = qty * price;
                    var total = self._convertString( self.storage.getItem( self.total ) );
                    var sTotal = total + subTotal - currsubt;
        ...
        _removeFromCart: function( index ) {
            var cart = this.storage.getItem( this.cartName );
